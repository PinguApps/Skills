...
$firstExpectedHeadEyesState = [pscustomobject]@{ seenReactionIds = @(); reviewStartedObserved = $false; approvalCandidateObserved = $false; expectedHeadReactionsCaptured = $false; reviewHeadBoundary = $null; reviewHeadBoundaryTrusted = $false }
$firstExpectedHeadEyesCaptured = Initialize-ExpectedHeadReactionBaseline -State $firstExpectedHeadEyesState -Snapshot (New-Snapshot -Reactions @($eyes)) -ExpectedSha "new-sha" -Reviewer $reviewer
Assert-Equal $true $firstExpectedHeadEyesCaptured "Eyes already present in the first expected-HEAD snapshot should establish the review boundary."
Assert-Equal $true $firstExpectedHeadEyesState.reviewHeadBoundaryTrusted "First-snapshot eyes absent from the pre-push baseline should be retained as pushed-review evidence."
